...
public class  Phone implements Comparable<Phone> {
  ...
  private String internal_memory; // 21
  ...
  public int compareTo(Phone otherPhone) {
    int announced = Integer.parseInt(this.announced.split(" ")[0]) - Integer.parseInt(getAnnounced().split(" ")[0]);
    if (announced == 0) {
      int brand = this.brand.toLowerCase().compareTo(getBrand().toLowerCase());
      if (brand==0) {
        return this.brand.toLowerCase().compareTo(getBrand().toLowerCase());
      }

      return brand;
    }
    return announced;
    //    return approx_price_EUR - otherPhone.getApprox_price_EUR();
  }
}
